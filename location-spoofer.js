...
(function () {
  ...
  var extractionUnparsedPassthroughDefault = DEFAULT_CONFIG.rawPassthrough;
  ...
  function normalizeConfig(input) {
    ...
    // rawPassthrough: on unknown iOS envelopes, return the ORIGINAL response
    // untouched instead of a rewrapped synthetic wloc frame (safer for
    // wifi_request_tile / dispatcher.arpc on modern iOS).
  ...
  function spoofAppleResponse(responseBytes, configInput) {
    ...
    // rawPassthrough (default true on modern iOS): a rewrapped synthetic wloc
    // frame is inaccurate for unknown endpoints (wifi_request_tile /
    // dispatcher.arpc) and can corrupt geod's view of the response. Return the
    // original bytes untouched instead — same net effect as failOpen, but
    // byte-exact. Set rawPassthrough=false to force the synthetic rewrap.
    if (config.rawPassthrough !== false) {
      return {
        response: responseBytes,
        payload: payloadOf(responseBytes),
        wifiCount: 0,
        cellCount: 0,
        kind: extraction ? extraction.kind + "-unpatched" : "raw-passthrough",
        prefix: "",
